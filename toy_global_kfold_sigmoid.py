...
# model definition
## gate
gate_coeffs = torch.tensor(config_json["gate_coeffs"]).to(torch.float32).to(DEVICE)
gate_train_coeffs = config_json["gate_train_coeffs"]
print('gate_coeffs:', gate_coeffs)
...
n_experts = len(gate_coeffs)
if not gate_train_coeffs:
    n_experts = sum(x != 0 for x in gate_coeffs)
train_data = CustomDataset(feature, target, weights)
dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=1)

model = MixtureOfExperts_GlobalGate(
    input_idx_matrix=input_idx_matrix,
    exp_architecture = exp_architecture,
    gate_coeffs=gate_coeffs,
    gate_train_coeffs=gate_train_coeffs,
    activation='sigmoid'
).to(DEVICE)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_data)):
        ...
        if not (epoch%patience):
            with torch.no_grad():
                ...
                gate_tmp = model.get_gate_coeffs().cpu().detach().numpy().reshape((1,-1))
        ...
        # hostogram of the classifier output
        fig = plt.figure(figsize=(9,6))
        fig.patch.set_facecolor('white')
        ax= fig.add_axes([0.15, 0.1, 0.78, 0.8])
        with torch.no_grad():
        ...
        hD = plt.hist(pred_norm[target[:, 0]==1],weights=w[target[:, 0]==1],
                      bins=bins,label='DATA', color='black', lw=1.5,
                      histtype='step', zorder=2)
        hR = plt.hist(pred_norm[target[:, 0]==0], weights=w[target[:, 0]==0],
                      color='#a6cee3', ec='#1f78b4',
                      bins=bins, lw=1, label='REFERENCE', zorder=1)
